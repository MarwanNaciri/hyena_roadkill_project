...
# Adults and subadults VS cubs (without unknwonws as adults)
...
## B. CARCASSES WITH COLLISION CERTAINTY SCORE >= 0.5 ------------------------
...
## C. CARCASSES WITH COLLISION CERTAINTY SCORE >= 0.75 ------------------------

# Count each age class ----------------------
...
# Adults and subadults VS cubs
dbinom(x = nb.ad+nb.sub, size = sum(obs1), prob = proba2[1,1] + proba2[1,2])
# p = 0.001
dbinom(x = nb.ad+nb.sub+nb.unkn, size = sum(obs2), prob = proba2[1,1] + proba2[1,2]) # with the unknown considered adults
# p < 0.001
