...
#   dplyr::select(ID, date_obs, herds_position, long, lat, collision_certainty_score_NEW, location_certainty_score, age, sex)
# 
# 
# # ~~~ b. For carcasses >=0.5 ---------------------------------------------------
# 
...
#          location_certainty_score > 0,
#          age %in% c("adult", "unknown", "subadult")) %>%
#   dplyr::select(ID, date_obs, herds_position, long, lat, collision_certainty_score_NEW, 
#                 location_certainty_score, age, sex)
# 
# 
# 
# 
...
#             data = hy_carcasses_herds_position, 
...
# A. Serengeti IV seasons with transit season ==================================
...
hy_carcasses_herds_position <- hy_carcasses %>%
  ...
  filter(age %in% c("adult", "unknown", "subadult"),
         herds_position_w_transit != "transit")
...
# Add variable "position of the migratory herds"
hy_carcasses_herds_position_high_certainty <- hy_carcasses_high_certainty %>%
  mutate(year = year(date_obs), 
         herds_position_w_transit = ifelse(date_obs %within% interval(ymd(paste0(year, "-08-01")), 
                                                                      ymd(paste0(year, "-10-31"))), 
                                           "north west", 
...
#lat
wilcox.test(formula = lat ~ herds_position_w_transit, 
            data = hy_carcasses_herds_position_high_certainty, 
            alternative = "two.sided", exact = TRUE)
# p = 0.02
#long
wilcox.test(formula = long ~ herds_position_w_transit, 
...
median <- hy_carcasses_herds_position_ad %>%
  group_by(herds_position_w_transit) %>%
  summarise(med_long = median(long),
            med_lat = median(lat))

median_high_certainty <- hy_carcasses_herds_position_high_certainty %>%
  group_by(herds_position_w_transit) %>%
  summarise(med_long = median(long),
            med_lat = median(lat))
...
hy_carcasses_herds_position_0.5 <- hy_carcasses_formatted_spatial_0.5 %>%
  mutate(herds_position = ifelse(date_obs %within% interval(ymd(paste0(year, "-06-01")), 
                                                              ymd(paste0(year, "-10-31"))), 
                                 "north west", "south east")) %>%
  dplyr::select(ID, date_obs, herds_position, long, lat, collision_certainty_score_NEW, location_certainty_score, age, sex)
...
# Add variable "position of the migratory herds"
hy_carcasses_herds_position <- hy_carcasses_formatted_spatial %>%
  mutate(herds_position = ifelse(date_obs %within% interval(ymd(paste0(year, "-06-26")), 
                                                              ymd(paste0(year, "-10-25"))), 
                                 "north west", 
                                 ifelse(date_obs %within% interval(ymd(paste0(year, "-04-26")), 
                                                                     ymd(paste0(year, "-06-25"))),"transit",
                                        ifelse(date_obs %within% interval(ymd(paste0(year, "-10-25")), 
...
hy_carcasses_herds_position_ad%>%
...
# D. Serengeti IV seasons no transit season ====================================
