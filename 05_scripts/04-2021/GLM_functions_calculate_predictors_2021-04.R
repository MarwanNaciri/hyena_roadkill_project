...
SegmentationDf <- function(roads.df, segment.length) {
  # Convert the real desired segment length into the weird length that the functions understands
  ...
  for (i in 1:length(list.ID)) {
    ...
    #     d <- distHaversine(coords.road.i.segmented[[length(coords.road.i.segmented)]][k,],
...
MidpointAndLengthSegment <- function(coords) {
  L <- 0
  for (i in 1:(nrow(coords) - 1)) {
    d <- distHaversine(coords[i,], coords[i+1,])  # distHarvesine calculates the shortest ditstance between 
  ...
  j <- 1
  sum.intervals <- 0
  interval <- distHaversine(coords[j,], coords[j+1,])
  midpoint <- c()
...
GetClosestRoadSegment <- function(carcasses.df, roads.df) {
  ...
  for (i in 1:nrow(carcasses.df)) {
    ...
    for (j in 1:nrow(roads.df.i)) {
      d <- distHaversine(carcasses.df[i , c('long', 'lat')], 
      ...
      if (d < d.min) { # if the distance between carcass j and road i is smaller 
        ...
        ID_closest_seg <- paste0(roads.df.i[j, 3],"_",roads.df.i[j, 2])
        d.min <- d
        long <- roads.df.i[j, 6]
        lat <- roads.df.i[j, 7]
        ID_road_seg_point <- paste0(roads.df.i[j, 4], "_", j)
      }
    }
...
AssignCarcassesToSegments <- function(table.glm, closest.segment) {
  ...
  colnames(table.glm) <- c("segment_ID", "road_importance", "nbr_carcasses", 
                           "long_midpoint", "lat_midpoint", "length", "dist_from_center")
  return(table.glm)
}
    ...
    for (j in 1:nrow(list.ID.i)) {
      ...
      if (nrow(coords.j) > 1) {
        for (k in 1:(nrow(coords.j) - 1)) {
          d <- distHaversine(coords.j[k,], coords.j[k+1,])/1000
          L <- L + d
        }
      }
    }
...
ShortestDistanceToRiverAndWaterbody <- function(table.glm, waterbodies.shp, rivers.shp) {
  ...
  waterbodies.df <- c()
  for (i in 1:length(waterbodies.shp@polygons)) {
    row.i <- data.frame(ID = waterbodies.shp@data[["osm_id"]][i], 
                        name = waterbodies.shp@data[["name"]][i],# code = waterbodies.shp@data[["code"]][i], 
                        width = NA,
                        type = waterbodies.shp@data[["fclass"]][i], 
                        long = as.numeric(waterbodies.shp@polygons[[i]]@Polygons[[1]]@coords[,1]), 
                        lat = as.numeric(waterbodies.shp@polygons[[i]]@Polygons[[1]]@coords[,2]))
    waterbodies.df <- rbind(waterbodies.df, row.i)
  }
  ...
  for (j in 1:length(rivers.shp@lines)) {
    row.j <- data.frame(ID = as.factor(rivers.shp@data[["osm_id"]][j]), 
                        name = rivers.shp@data[["name"]][j],
                        width = rivers.shp@data[["width"]][j], 
                        type = NA, # For some reason, the file I'm using now doesn't have this info
                        long = as.numeric(rivers.shp@lines[[j]]@Lines[[1]]@coords[,1]), 
                        lat = as.numeric(rivers.shp@lines[[j]]@Lines[[1]]@coords[,2]))
    rivers.df <- rbind(rivers.df, row.j)
  }
  ...
  for (i in 1:nrow(table.glm)) {
    water.df.i <- water.df %>%                                # This is just to reduce the computational burden:
      filter(abs(long - table.glm$long_midpoint[i]) < 0.164,  # for each segment I only consider water points that 
             abs(lat - table.glm$lat_midpoint[i]) < 0.164)    # are less than 0.164 degrees (latitude and longitude) away
    ...
    for (j in 1:nrow(water.df.i)) {
      d <- distHaversine(table.glm[i,4:5], water.df.i[j,5:6])
      if (d < d.min) {
        d.min <- d
        j.min <- j
      }
    ...
    closest.water <- rbind(closest.water, c(ID_segment, 
                                            ...
                                            water.df.i$type[j.min], water.df.i$name[j.min], 
                                            water.df.i$width[j.min]))
  }
  closest.water <- as.data.frame(closest.water)
  colnames(closest.water) <- c("segment_ID", "ID_closest_water", "distance", "long_water", "lat_water", "type", "name", "width")
...
# ShortestDistanceAmenity <- function(table.glm, buildings.polygons.shp, buildings.points.shp) {
#   ### Create a dataframe with the osm ID, name, type and coordinates of polygon buildings
#   buildings.polygons.df <- c()
#   for (i in 1:length(buildings.polygons.shp@polygons)) {
#     row.i <- data.frame(ID = buildings.polygons.shp@data[["osm_id"]][i], 
#                         type = as.factor(ifelse(buildings.polygons.shp@data[["building"]][i] %in% c("yes", "commercial"), "building",
#                                                 ifelse(buildings.polygons.shp@data[["building"]][i] %in% c("tent"), "tent",
#                                                        ifelse(buildings.polygons.shp@data[["building"]][i] %in% c("hut"), "hut",
#                                                               ifelse(buildings.polygons.shp@data[["tourism"]][i] %in% c("hotel"), "building",
#                                                                      ifelse(buildings.polygons.shp@data[["tourism"]][i] %in% c("hostel", "camp_site"), "camp_site",
#                                                                             ifelse(buildings.polygons.shp@data[["amenity"]][i] %in% c("dining_room", "kitchen", "toilets", "police"), "building",
#                                                                                    "picninc_site"))))))),
#                         name = buildings.polygons.shp@data[["name"]][i],
#                         long = buildings.polygons.shp@polygons[[i]]@Polygons[[1]]@coords[,1], 
#                         lat = buildings.polygons.shp@polygons[[i]]@Polygons[[1]]@coords[,2])
#     buildings.polygons.df <- rbind(buildings.polygons.df, row.i)
#   }
#   ### Create a dataframe with the osm ID, name, type and coordinates of polygon points
#   buildings.points.df <- c()
#   for (i in 1:nrow(buildings.points.shp@coords)) {
#     row.i <- data.frame(ID = buildings.points.shp@data[["osm_id"]][i], 
#                         type = as.factor(ifelse(buildings.points.shp@data[["building"]][i] %in% c("school"), "building",
#                                                 ifelse(buildings.points.shp@data[["office"]][i] %in% c("ngo", "government"), "building",
#                                                        ifelse(buildings.points.shp@data[["shop"]][i] %in% c("kiosk", "newsagent", "tyres", "yes"), "building",
#                                                               ifelse(buildings.points.shp@data[["amenity"]][i] %in% c("bar", "toilets", "fuel", "doctors", "cafe", "atm"), "building",
#                                                                      ifelse(buildings.points.shp@data[["tourism"]][i] %in% c("guest_house", "hotel", "apartment", "caravan_site", "chalet", "information"), "building",
#                                                                             ifelse(buildings.points.shp@data[["name"]][i] %in% c("Rhino Post", "Naabi Gate"), "building",
#                                                                                    ifelse(buildings.points.shp@data[["tourism"]][i] %in% c("camp_site"), "camp_site",
#                                                                                           ifelse(buildings.points.shp@data[["tourism"]][i] %in% c("picnic_site"), "picnic_site",
#                                                                                                  ifelse(buildings.points.shp@data[["osm_id"]][i] %in% c("1453292037"), "picnic_site",
#                                                                                                         "building")))))))))),
#                         name = buildings.points.shp@data[["name"]][i],
#                         long = buildings.points.shp@coords[i,1],
#                         lat = buildings.points.shp@coords[i,2])
#     buildings.points.df <- rbind(buildings.points.df, row.i)
#   }
#   buildings.points.df <- buildings.points.df %>%
...
#   infrastructure.df <- rbind(buildings.polygons.df, buildings.points.df)
...
#       d <- distHaversine(table.glm[i,4:5], infrastructure.df.i[j,4:5])
#       if (d < d.min) {
#         d.min <- d
#         j.min <- j
...
ShortestDistanceAmenity <- function(table.glm, buildings.polygons.shp, buildings.points.shp) {
  ...
  ### Create a dataframe with the osm ID, name, type and coordinates of points buildings
  buildings.points.df <- c()
  for (i in 1:nrow(buildings.points.shp@coords)) {
    row.i <- data.frame(ID = buildings.points.shp@data[["osm_id"]][i], 
                        name = buildings.points.shp@data[["name"]][i],
  ...
  closest.infrastructure <- c()
  for (i in 1:nrow(table.glm)) {
    infrastructure.df.i <- infrastructure.df %>%             # This is just to reduce the computational burden:
      filter(abs(long - table.glm$long_midpoint[i]) < 0.28,  # for each segment I only consider buildings that
             abs(lat - table.glm$lat_midpoint[i]) < 0.28)    # are less than 0.28 degrees (latitude and longitude) away
    ...
    for (j in 1:nrow(infrastructure.df.i)) {
      d <- distHaversine(table.glm[i, c("long_midpoint", "lat_midpoint")], infrastructure.df.i[j,3:4])
      if (d < d.min) {
        d.min <- d
        j.min <- j
      }
    }
    closest.infrastructure <- rbind(closest.infrastructure, 
                                    c(ID_segment,
                                      infrastructure.df.i$ID[j.min], 
                                      d.min, 
                                      infrastructure.df.i$long[j.min], 
  ...
  colnames(closest.infrastructure) <- c("segment_ID", "ID_closest_infrastructure", 
                                        "distance", "long_infrastructure", 
                                        "lat_infrastructure") #, "type", "name")
  closest.infrastructure <- as.data.frame(closest.infrastructure)
  return(closest.infrastructure)
}
...
BuildingDensity <- function(table.glm, building.polygons.shp, building.points.shp) {
  ...
  buildings.df <- rbind(building.polygons.df, building.points.df)
  # Building density
  building.density <- c()
  for (i in 1:nrow(table.glm)) {
    L <- 0
    buildings.df.i <- buildings.df %>%
      filter(abs(long - table.glm$long_midpoint[i]) < 0.045,
             abs(lat - table.glm$lat_midpoint[i]) < 0.045)
    list.ID.i <- buildings.df.i %>%
      distinct(ID)
    N <- nrow(list.ID.i)
...
ExtractLandCover <- function(roads.segmented.df, land.cover, buffer.size) {
  ...
  land.cover.types <- c(0, 1, 2, 3, 6, 7, 8, 9, 10, 11, 12, 13, 15, 16, 17, 20, 23, 24, 25, 
                         27, 28, 29, 32, 35, 36, 39, 40, 41)
