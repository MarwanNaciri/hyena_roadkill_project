#==============================================================================#
#                                                                              #
#             Analyses with only carcasses handled by hyena project            #        
#                                                                              #
#==============================================================================#
...
handled_by_hyena <- handler %>%
  filter(handled_by_hyena_project == "yes")
...
hy_carcasses <- read_delim("06_processed_data/carcasses/12_hy.carcasses.certainty.formatted.spatial_updated_10-2021.csv", 
  ...
  filter(ID_OG %in% handled_by_hyena$ID_OG)
...
# p = 0.019, same thing because no unknown
...
hy_carcasses_clan_members <- read_delim("06_processed_data/carcasses/3_hy.clan.members.csv", 
                                        ";", escape_double = FALSE, trim_ws = TRUE) %>%
  filter(ID %in% handled_by_hyena$ID,
         sex == "F",
...
# Statistical tests
# Only three clan members manipulated by hyena project
...
handled_by_hyena <- read_delim("06_processed_data/carcasses/15_finder_of_carcasses_justified.csv", 
                      delim = ";", escape_double = FALSE, trim_ws = TRUE) %>%
  filter(handled_by_hyena_project == "yes")
...
write_csv(table.glm_5, "06_processed_data/glm_data_2021-10/table.glm_5_handled_by_hyena.csv")
