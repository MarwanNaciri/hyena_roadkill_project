...
ggplot(df_glm, aes(x = parameter, y = mean)) +
  ...
  scale_x_discrete(limits = rev(c("(Intercept)", 
                                  "road_importancemajor_road", "distance_amenity_km", 
                                  "distance_water_km", "woodland")),
...
save_plot("11_manuscript/V3 Figures/figure 2 raw.svg", 
          plot = figure.GLM,
          ncol = 2,
          nrow = 2,
          # base_asp = 1.618,
...
table.glm.0.75 <- read_csv(paste0("06_processed_data/glm_data_2021-10/table.glm_9.", 
...
# To rescale the predictor
mean_sd <- read_csv(paste0("06_processed_data/glm_data_2021-10/predictor_mean_sd_", buffer.size, "m.csv"))
x <- which(mean_sd$variable == "distance_amenity_km")
y <- newdata_distance_amenity$distance_amenity_km * mean_sd$sd[x] + mean_sd$mean[x]
range(y)
(plt.dist.amenity <- ggplot() +
