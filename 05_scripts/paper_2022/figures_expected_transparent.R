...
expected_counts <- data.frame(month = seq(1:12),
                              n = c(31/365, 28/365, 31/365, 30/365, 31/365, 30/365, 
...
df_plot_month <- hy_carcasses_month %>%
    mutate(herds_position = ifelse(month %in% c(1, 2, 3, 4, 5, 12), "south_east",
                                   ifelse(month %in% c(8, 9, 10), "north_west", "transit")),
           month_name = c("Jan", "Feb", "Mar", "Apr", "May", "Jun", 
                          "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"))
...
ggplot() +
  geom_bar(data = df_plot_month, aes(x = month, y = n, fill = herds_position),
           stat = "identity",
           position = position_dodge(),
           color = "black",
           size = 0.5) +
  geom_bar(data = expected_counts, aes(x = month, y = n, fill = herds_position),
           stat = "identity",
           position = position_dodge(),
           color = "black",
           alpha = 0.5,
...
(plot.age <- ggplot(df.age, aes(x = age, y = n, alpha = type)) +
    geom_bar(stat = "identity",
             position = position_dodge(),
             color = "black",
             size = 0.5, fill = "#E69F00") +
    # scale_fill_grey(start = 0.8, end = 0.2) +
    scale_alpha_discrete(limits = c("expected", "observed"), 
                         range = c(0.3, 1)) +
    theme_classic() +
...
clan_size_year_clan <- read_csv("06_processed_data/hyenas/Clan_size_year_clan.csv", 
  ...
  group_by(age_class) %>%
  summarise(mean_nb = mean(nb))
proportion_age_classes_raw <- data.frame(age_class = clan_size_year_clan_new$age_class,
                                         proportion = clan_size_year_clan_new$mean_nb/sum(clan_size_year_clan_new$mean_nb))
...
ggsave("11_manuscript/V4 Figures/figure 4 (age, sex and rank).png",
       width = 7, height = 2.75)
...
ggsave("11_manuscript/V4 Figures/figure S1 (sampling effort).png",
       width = 9, height = 3)
